...
class ReportRepository
{
    ...
    public function getSummaryData($params)
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            $list[] = array(
                ...
                'batch'                 => $transaction->studentFeeRecord->studentRecord->batch->course->name,
                'head'                  => $head,
    ...
    private function getHeadDetail(Transaction $transaction, $params = array())
    {
        ...
        if ($transaction->income_id) {
            ...
            /**(this is original record, below is modified data) $head = trans('finance.fee').' '.$students->firstWhere('id', $student_record->student_id)->name.' ('.$student_record->admission->admission_number.')';*/

            $head = trans('Fee;').' '.$students->firstWhere('id', $student_record->student_id)->name.' - '.$student_record->admission->admission_number.' ('.$student_record->batch->course->academic_session_id.') ('.$student_record->batch->course->name.'-'.$student_record->batch->name.')';

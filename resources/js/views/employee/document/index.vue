    ...
    <div>
        ...
        <div class="table-responsive" v-if="employee_documents.total">
            <table class="table table-sm">
                <thead>
                    <tr>
                        <th>{{trans('employee.document_title')}}</th>
                        <th>{{trans('employee.document_type')}}</th>
                        <th>{{trans('general.updated_at')}}</th>
                        <th class="table-option">{{trans('general.action')}}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="employee_document in employee_documents.data">
                        <td v-text="employee_document.title"></td>
                        <td v-text="employee_document.employee_document_type.name"></td>
                        <td>{{employee_document.updated_at | momentDateTime}}</td>
...
<script>
    ...
    export default {
        ...
        methods: {
            editAction(employee_document){
                this.editId = employee_document.id;
                this.editModal = true;
            },
            showAction(employee_document){
                this.showId = employee_document.id;
                this.showModal = true;
            },
            getEmployeeDocuments(page){
                let loader = this.$loading.show();
                if (typeof page !== 'number') {
                    page = 1;
                }
                let url = helper.getFilterURL(this.filter);
                axios.get('/api/employee/'+this.employee.uuid+'/document?page=' + page + url)
                    .then(response => {
                        this.employee_documents = response;
                        loader.hide();
                    })
                    .catch(error => {
                        loader.hide();
            ...
            confirmDelete(employee_document){
                return dialog => this.deleteEmployeeDocument(employee_document);
            },
            deleteEmployeeDocument(employee_document){
                let loader = this.$loading.show();
                axios.delete('/api/employee/'+this.employee.uuid+'/document/'+employee_document.id)
                    .then(response => {

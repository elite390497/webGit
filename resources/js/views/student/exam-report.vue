    ...
    <div v-if="student.id">
        ...
        <div class="table-responsive" v-if="rows.length">
            <table class="table table-sm table-bordered">
                <thead>
                    <tr>
                        <th class="text-center" v-for="head in header" :key="head.key">{{head.label}}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="row in rows" :key="row.key">
                        <td v-for="column in row" :key="column.key" class="text-center">
                            <span v-if="column.detail">
                                <span style="display: block;" v-for="detail in column.detail">
                                    <span v-if="detail.mark">{{detail.assessment_detail_code}}: {{detail.mark+'/'+detail.assessment_detail_max_mark}}</span>
                                </span>
                            </span>
                            <span v-else>
                                <span v-if="column.no_exams">-</span>
    ...
    export default {
        ...
        methods: {
            fetch(student_record) {
                ...
                axios.get('/api/student/'+this.student.uuid+'/record/'+student_record.id+'/exam')
                    .then(response => {

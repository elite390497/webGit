...
<div style="@if((isset($print_options) && ! gbv($print_options, 'no_border')) || ! isset($print_options)) border:1px dashed #696969; @endif margin-top: 10px;">
    ...
    <table id="myTable" class="table feeinstallment table-striped fancy-detail bordercustom">
        ...
        <tr style="text-transform: uppercase; text-align: center">
            <th class="bordercustom">{{trans('finance.receipt_no')}}</th>
            <th class="bordercustom">{{trans('student.admission_number_short')}}</th>
            <th class="bordercustom">{{trans('student.name')}}</th>
            <th class="bordercustom">{{trans('academic.batch')}}</th>
        <!-- <th class="bordercustom">{{trans('student.contact_number')}}</th>-->
        <!--<th class="bordercustom">{{trans('finance.amount')}}</th>-->
            <th class="bordercustom">Paid Amount</th>
            <th class="bordercustom">{{trans('finance.fee_installment')}}</th>
            <th class="bordercustom">Fee Type</th>
            <th class="bordercustom">{{trans('finance.date')}}</th>
            <th class="bordercustom">{{trans('Academic Session')}}</th>
            <th class="bordercustom">{{trans('finance.payment_method')}}</th>
            <th class="bordercustom">{{trans('Remarks')}}</th>
        </tr>
        ...
        <!--<th class="bordercustom">{{trans('student.contact_number')}}</th>-->
            ...
            <th class="bordercustom dropdown-header" data-field-name="feetype"></th>
            <th class="bordercustom dropdown-header" data-field-name="date"></th>
            <th class="bordercustom dropdown-header" data-field-name="academic_session"></th>
            <th class="bordercustom dropdown-header" data-field-name="payment"></th>
            <th class="bordercustom dropdown-header" data-field-name="remarks"></th>
        </tr>
        </thead>
        <tbody>
        @foreach($list as $item)
            <tr class="row">
                <td class="bordercustom">{{ $item['receipt_no'] }}</td>
                <td class="bordercustom">{{ $item['admission_number'] }}</td>
                <td class="bordercustom">{{ $item['name'] }}</td>
                <td class="bordercustom" data-field-name="batch">{{ $item['batch'] }}</td>
            <!--<td class="bordercustom">{{ $item['contact_number'] }}</td>-->
                <td class="bordercustom">{{ currency($item['amount']) }}</td>
                <td class="bordercustom" data-field-name="term">{{ $item['fee_installment'] }}</td>
                <td class="bordercustom" data-field-name="feetype">{{ $item['fee_group'] }}</td>
                <td class="bordercustom" data-field-name="date">{{ $item['date'] }}</td>
                <td class="bordercustom" data-field-name="academic_session">{{ $item['academic_session'] }}</td>
            <!--<td class="bordercustom" data-field-name="academic_session">{{ $item['account'] }}</td>-->
                <td class="bordercustom" data-field-name="payment">{{ $item['payment_method'] }}</td>
                <td class="bordercustom" data-field-name="remarks">{{ $item['remarks'] }}</td>
        ...
        <tr>
            <th colspan="4"></th>
            <th class="total">{{ $footer['grand_total'] }}</th>
            <th colspan="6"></th>
        </tr>
        </tfoot>
    </table>

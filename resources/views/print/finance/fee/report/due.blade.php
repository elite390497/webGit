...
<div style="@if((isset($print_options) && ! gbv($print_options, 'no_border')) || ! isset($print_options)) border:1px dashed #696969; @endif margin-top: 10px;">
    ...
    <table id="myTable" class="table feeinstallment table-striped fancy-detail bordercustom">
        ...
        <tr style="text-transform: uppercase; text-align: center">
            ...
            <th class="bordercustom">{{trans('finance.total')}}</th>
            <th class="bordercustom">{{trans('finance.fee_installment_due_date')}}</th>
            <th class="bordercustom">{{trans('finance.fee_overdue')}}</th>
            <th class="bordercustom">{{trans('finance.late_fee')}}</th>
        </tr>
        <tr style="text-transform: uppercase; text-align: center" class="mediaprintcustom">
            ...
            <th class="bordercustom"></th>
            <th class="bordercustom"></th>
            <th class="bordercustom dropdown-header" data-field-name="duedate"></th>
        ...
        @foreach($list as $item)
            <tr class="row">
                ...
                <td class="bordercustom">{{ $item['first_guardian_name'] }}</td>
                <td class="bordercustom">{{ $item['contact_number'] }}</td>
                <td class="bordercustom">{{ $item['total'] }}</td>
                <td class="bordercustom" data-field-name="duedate">{{ $item['due_date'] }}</td>
                <td class="bordercustom" data-field-name="date">{{ trans('finance.fee_overdue_day', ['day' => $item['overdue']]) }}</td>
                <td class="bordercustom">{{ $item['late_fee'] }}</td>
            </tr>
        @endforeach
        </tbody>
        <tfoot>
    ...
    <script>
        ...
        function filterText(e) {
            ...
            for(var i = 2;i<=numOfVisibleRows.length-3+1;i++) {
                ...
                result = result + parseInt(numOfVisibleRows[i].cells[5].innerHTML); //change the cell number for total
            }
            console.log(result);
            $(".total").html(result);
        }
